...
-   New features and user-visible changes:
    ...
    -   the output of parTable(fit) now fully reflects the changes described [here](http://lavaan.ugent.be/notes/lavaan_eq_constraints.pdf)
...
-   New features and user-visible changes:
    -   huge improvement in speed and stability when (many) linear equality constraints are used in the model; the details of the changes are described [here](http://lavaan.ugent.be/notes/lavaan_eq_constraints.pdf)
    -   new function lavPredict() to compute predicted values for latent variables and observed variables
    -   many low-level lav_matrix\_\* functions are now public
